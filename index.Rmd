...
lm.cleare.ols <- lm(log(mean.in)~year, data=subset(avg.lengths, area=="capecleare"))

lm.junken.ols <- lm(log(mean.in)~year, data=subset(avg.lengths, area=="junken"))

lm.aialik.ols <- lm(log(mean.in)~year, data=subset(avg.lengths, area=="aialik"))

lm.resbay.ols <- lm(log(mean.in)~year, data=subset(avg.lengths, area=="resbay"))
```
...
lm.el.wls <- lm(log(mean.in)~year, data=subset(avg.lengths, area=="elrington"), weights=subset(weights, area=="elrington")$n)

lm.cleare.wls <- lm(log(mean.in)~year, data=subset(avg.lengths, area=="capecleare"), weights=subset(weights, area=="capecleare")$n)

lm.junken.wls <- lm(log(mean.in)~year, data=subset(avg.lengths, area=="junken"), weights=subset(weights, area=="junken")$n)

lm.aialik.wls <- lm(log(mean.in)~year, data=subset(avg.lengths, area=="aialik"), weights=subset(weights, area=="aialik")$n)

lm.resbay.wls <- lm(log(mean.in)~year, data=subset(avg.lengths, area=="resbay"), weights=subset(weights, area=="resbay")$n)
```
...
a <- ggplot(subset(avg.lengths, area=="aialik"), 
  ...
  geom_point(aes(as.numeric(as.character(year)), log(mean.in)), size=4)+
  geom_line(aes(as.numeric(as.character(year)), log(mean.in)), linetype="dotted")+
...
e <- ggplot(subset(avg.lengths, area=="capecleare"), 
  ...
  geom_point(aes(as.numeric(as.character(year)), log(mean.in)), size=6)+
  geom_line(aes(as.numeric(as.character(year)), log(mean.in)), linetype="dotted")+
  theme(legend.position = "none")+
  xlab("Cape Cleare")+
  ylab("log(Length)")+
